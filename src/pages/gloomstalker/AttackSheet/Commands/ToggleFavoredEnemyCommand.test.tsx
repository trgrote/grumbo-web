...
describe('ToggleFavoredEnemyCommand', () => {
	it('adds the name to selectedFavoredEnemies when not already selected', () => {
		...
		expect(new ToggleFavoredEnemyCommand('Giant').apply(state).character.selectedFavoredEnemies).toEqual(['Goblin', 'Giant']);
	});
	...
	it('removes the name from selectedFavoredEnemies when already selected', () => {
		...
		expect(new ToggleFavoredEnemyCommand('Giant').apply(state).character.selectedFavoredEnemies).toEqual(['Goblin']);
	});
});
