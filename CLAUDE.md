...
- `AttackSheet.tsx` is the whole sheet: the trigger button, the sliding panel, the model/reducer wiring, the step-entry and completion effects, and the two reset paths — closing the sheet, and the character's `info` changing. That second one keys on `info` rather than the memoized model on purpose: `useMemo` isn't guaranteed to keep its cache, and a dropped one would reset an attack already in progress. `createModel` is kept out of that memo's dependencies to avoid rebuilding the model every render — a cost argument, not a correctness one — which makes it the caller's job to keep `createModel` a pure function of `info`, since anything else it closes over is captured once. In dev, the sheet logs an error if the model declares a step the map has no component for, since that renders a panel with no way out but discarding the attack. A character supplies `info`, a `createModel` factory, a `steps` map of `AttackStep` → component, and `onAttackComplete`; optionally `onStepEntered` for side effects on landing on a step (the Paladin plays a sound on a crit). Side effects belong here rather than on `ICharacterAttackModel`, whose methods are pure transforms driven by an injected rng.
...
- `<Character>AttackSheet.tsx` is now thin: it declares the `AttackStep` → component map (which should mirror the model's `steps` array) and renders the shared `AttackSheet`, turning the completed state into a `HistoryRecord` in `onAttackComplete`. That record is what actually gets persisted, via the page's `useEffect` → `SaveLocal*Storage`. It holds no reducer wiring, no effects and no Sheet markup of its own.

**`HistoryRecord` is deliberately flat**, not nested like the sheet state — `CreateHistoryRecordFromState` spreads `characterState` up to the top level. That's what lets records persisted before the refactor keep deserializing without a `storageVersion` bump. Don't "fix" the inconsistency without bumping the version and accepting the history loss.

**It deliberately does not carry `attackStep`**, and nothing persisted should. A finished attack is always on the final step, so the field was write-only — and an enum ordinal is an unsafe thing to store: adding `PostDamageRoll` to the shared enum reassigned `3` from `Results` to `PostDamageRoll` under every Paladin record already on disk. If you change a character's flow, nothing on disk should have to be renumbered. Both integration tests assert the record has no `attackStep`.
...
Vitest + jsdom, configured inline in `vite.config.ts` (`test: { environment: 'jsdom', setupFiles: ['./src/test/setup.ts'] }`). Tests are co-located as `*.test.tsx` next to the source file, one per command / state-function / model module — see `AttackSheet/Commands/*.test.tsx`, `AttackSheet/AttackSheetStateFunctions.test.tsx` and `AttackSheet/*AttackModel.test.tsx` in either character page for the convention. The reducer lives in the shared module and is tested there.
